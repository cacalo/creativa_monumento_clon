...
const ficha = getParameterByName('ficha');
...
const url = document.querySelector("meta[property='json']").getAttribute("content");
console.log(url)
...
.on('click', function(d) {
    const audio = new Audio("./audios/"+d.audio);
    const vinculoInterno = d.vinculoInterno;
    const vinculoExterno = d.vinculoExterno;
    const video = d.videoYT;
    console.log(video, "http://www.youtube.com/embed/"+video+"?autoplay=1")
